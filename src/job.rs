...
impl Job {
    /// host_port is generated by this host.
    /// restrict_net, if true, will only allow the host to access the VM,
    /// and the VM cannot access the broader internet.
    pub fn qemu_cmd(&self, host_port: u16, restrict_net: bool) -> std::process::Command {
        let mut cmd = std::process::Command::new(self.arch.qemu_binary());

        cmd.arg("-name").arg(&self.name);
        cmd.arg("-m").arg(format!("{}M", self.memory));
        cmd.arg("-smp").arg(self.cpus.to_string());
        cmd.arg("-drive")
            .arg(format!("file={},format=qcow2", self.image));
        cmd.arg("-display").arg("none");
        cmd.arg("-accel").arg("kvm:hvf:tcg");

        let netdev = if restrict_net {
            format!(
                "user,id=net0,restrict=yes,hostfwd=tcp::{}-:{}",
                host_port, self.port
            )
        } else {
            format!("user,id=net0,hostfwd=tcp::{}-:{}", host_port, self.port)
        };
