...
bool get_process_start_time_native(uint32_t pid, uint64_t *out_start_time) {
  ...
  BOOL success = GetProcessTimes(handle, &creation_time, &exit_time,
  ...
  *out_start_time = ((uint64_t)creation_time.dwHighDateTime << 32) |
                    creation_time.dwLowDateTime;
  return true;
}

uint64_t start_time_to_unix_secs(uint64_t start_time) {
  // start_time is FILETIME (100-nanosecond intervals since Jan 1, 1601) on
  // windows for some reason 1601 to 1970 in 100-nanosecond intervals
